    ...
    { 
        public static void Prefix(BuildingInfo __instance)
        {
            try
            {
                ...
                else if (__instance.m_class.m_service == ItemClass.Service.HealthCare &&  (__instance.name.Contains("Aqua Crop Extractor") || __instance.name.Contains("Aqua Fish Extractor")) && __instance.GetAI() is not AquaExtractorAI)
                {
                    var oldAI = __instance.GetComponent<PrefabAI>();
                    Object.DestroyImmediate(oldAI);
                    var newAI = (PrefabAI)__instance.gameObject.AddComponent<AquaExtractorAI>();
                    PrefabUtil.TryCopyAttributes(oldAI, newAI, false);
                }
            }
            catch (Exception e)
