...
namespace IndustriesMeetsSunsetHarbor.UI
{
    public class ExtendedWarehouseWorldInfoPanel : BuildingWorldInfoPanel
    {
        ...
        protected override void OnSetTarget()
        {
            ...
            m_resourcePanel.isVisible = extendedWarehouseAI.m_storageType == ExtendedTransferManager.TransferReason.None;
        ...
        protected override void UpdateBindings()
        {
            ...
            ExtendedWarehouseAI extendedWarehouseAI = building.Info.m_buildingAI as ExtendedWarehouseAI;
            ...
            int num = building.m_customBuffer1 * 100;
            m_resourceProgressBar.value = (float)num / (float)extendedWarehouseAI.m_storageCapacity;
            ExtendedTransferManager.TransferReason transferReason = extendedWarehouseAI.GetTransferReason(m_InstanceID.Building, ref Singleton<BuildingManager>.instance.m_buildings.m_buffer[m_InstanceID.Building]);
            ExtendedTransferManager.TransferReason actualTransferReason = extendedWarehouseAI.GetActualTransferReason(m_InstanceID.Building, ref Singleton<BuildingManager>.instance.m_buildings.m_buffer[m_InstanceID.Building]);
            m_resourceProgressBar.progressColor = Color.Lerp(Color.grey, Color.black, 0.2f);
            m_resourceLabel.text = actualTransferReason.ToString();
            m_emptyingOldResource.isVisible = transferReason != actualTransferReason;
            m_resourceDescription.isVisible = transferReason != ExtendedTransferManager.TransferReason.None;
            m_resourceDescription.text = GenerateResourceDescription(transferReason, isForWarehousePanel: true);
            m_resourceSprite.spriteName = extendedWarehouseAI.m_storageType.ToString();
            ...
            m_Info.text = extendedWarehouseAI.GetLocalizedStats(buildingId, ref instance.m_buildings.m_buffer[buildingId]);
            if (extendedWarehouseAI != null)
            {
                UpdateWorkers(buildingId, extendedWarehouseAI, ref instance.m_buildings.m_buffer[buildingId]);
            }
        }
        ...
        public static string GenerateResourceDescription(ExtendedTransferManager.TransferReason resource, bool isForWarehousePanel = false)
        {
